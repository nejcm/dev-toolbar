...
| Diagnostics roster | `readDiagnosticsRoster`: 1; `redactForExport`: 1 | Admitted as an explicit exception, below. `/ext/diagnostics` reads core's roster through it, and its consumer `sources` go through `redactForExport`. `/ext/agent` still masks the roster its own way; when it adopts both, for the roster and for its command results, they read 2 and 2. |
