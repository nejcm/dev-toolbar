...
*Kinds* are the shared vocabulary: `action`, `chip`, `dot`, `label`, `value`, `note`,
`tag`, `row`, `rows`, `list`, `empty`, `banner`, `search`, `toolbar`, `field`. They are
shared across extensions on purpose, so one rule of yours reaches every panel button or every
key/value grid at once — something part names, being namespaced, cannot do. `KIT_CSS`,
