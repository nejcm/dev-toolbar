 ...
 * not a string, or the anchored `redact()` pass over the whole text threw.
 * Replaces the *whole* text, so nothing uninspectable is shown; the URL sweep
 * that follows is a separate, per-match concern — see `redactProse`.
 */
 ...
 * matching `author`.
 *
 * Whole-word entries (`authorization`, `bearer`, `token`, `secret`, …) match
 * that segment wherever it sits. Concatenation entries (`apikey`,
 * `sessionid`, `accesstoken`, `jsessionid`, …) exist for run-together keys
 * with no separator or case boundary, which segment to one piece and match
 * only because the exact concatenation is listed — a compound not listed
 * (`bearertoken`) is a gap to fill via `extraKeys`. A few entries
 * (`clientsecret`, `refreshtoken`) are redundant with `secret`/`token` but
 * kept to document the shapes this list targets.
 */
...
export interface RedactOptions {
   ...
   * just `"session"` doesn't exempt `sessionName` either. The trailing-`s`
   * tolerance `keys`/`extraKeys` get isn't extended here either — name the
   * exact key shape you mean to exempt.
   */
   ...
   * same way as `maxDepth`.
   *
   * `maxDepth`/`maxArrayLength` bound a single path, but not how many times a
   * **shared** reference is walked: the cycle guard is scoped per-path, so a
   * DAG where several keys alias the same child costs `k^d` walks for `k`
   * keys at each of `d` levels — `k=8, d=7` measured 1268 ms and ~2.1M walks,
   * within `maxDepth: 8`'s default. `maxNodes` caps that shared total
   * instead, without changing correctness. Once spent, every remaining node
   * (sibling keys included) becomes `"[truncated]"` too.
   */
  maxNodes?: number;
  /**
   * Also mask string *values* that look like credentials regardless of key:
   * `Bearer …`/`Basic …`/`Digest …` scheme headers, bare JWTs, and absolute
   * URLs (any registered scheme) with a sensitive query/fragment parameter —
   * the OAuth-callback shape, where key matching can't find the secret.
   * Relative references and non-URL header values are out of scope here —
   * call `redactUrl()` on those. Default `true`. A URL with nothing to mask
...
// `APIKe`/`y`. Unicode properties (not `[A-Za-z]`) so `contraseña`, `пароль`,
// `密码` segment correctly too.
 ...
 * run of segments, which can only contain segment characters.
 */
 ...
 *
 * Runs are built from every start position and looked up in the entry set
 * rather than looping the set per run, so cost scales with the key's length
 * times the longest entry, not the list size — keeps a hostile key cheap
 * (`"a-".repeat(50000)` took 160ms with the old per-entry loop).
 *
 ...
 * `fallback` for anything that isn't one. Without this, `maxArrayLength: NaN`
 * reaches `new Array(NaN)`, which throws — defeating the guarantee that
 * `redact()` never throws. Falls back to the default rather than "unbounded",
 * since an unbounded node budget is the exploit `maxNodes` exists to close.
 */
 ...
 * called per-header or per-key in a hot loop.
 *
 * No options resolves once into a module-level constant; an options object is
 * cached in a `WeakMap` keyed on identity, not contents, so a reused options
 * object resolves once but a fresh literal each call gets no reuse.
 * Resolution is snapshot-once: mutating an already-cached options object's
 * fields doesn't change what later calls with it see.
 */
 ...
 *
 * The rule: an entry matches when it equals one or more adjacent whole word
 * segments of the key, give or take a trailing `s`. The key is split on
 * separators and case/letter-digit boundaries, so `X-Auth-Token`,
 * `authToken`, `auth_token` all segment to `["auth", "token"]`. `allowKeys`
 * wins and is an exact match against the closed-up segments, not a segment
 * match — see `RedactOptions.allowKeys` for the asymmetry.
 *
 ...
 * `pin` matched `spinner`) — forced `/ext/theme-editor` to route
 * `--sidebar-bg`/`--spinner-size` around it.
 */
...
// The lookbehind keeps it linear on a long alphanumeric run. Closing
// delimiters `)`/`]`/`.,;:!?` end the match only as its last character, so a
// wrapped URL stops cleanly without truncating `?ids[]=1&token=…` at the
// first `]`. Byte-identical to `URL_IN_TEXT` in
...
export function redactText(text: string, options?: RedactTextOptions): string {
  ...
  if (resolved.values) {
    ...
    // Digest has no reliable end delimiter in prose, so discard its entire suffix.
  ...
  // With `url`, the whole input is tried as one URL — keeps the precise rewrite
  // (`?token=[redacted]`) where the value really is one URL. The embedded-URL
  // scan still runs as well, since a value with whitespace parses as one URL
  // that can hide a second URL's credentials from `maskUrl`. Where both find
  // something the merge below collapses them to a plain mask; an identical
  // whole-range match is skipped so the single-URL case keeps its rewrite.
  let whole = false;
  if (options?.url === true) {
...
function redactString(value: string, resolved: ResolvedOptions): string {
  ...
  // since this is a redactor, not a canonicaliser.
 ...
 * it — a value replaced by a lie is what this module exists to prevent.
 */
 ...
 *
 * Three overloads, matching what `walk` actually guarantees: string in/out,
 * primitives returned by identity, and anything else typed `unknown` (a
 * plain object usually comes back as `Record<string, unknown>`, but a
 * cycle/`maxDepth: 0`/hostile Proxy makes it a tag string instead).
 *
 * The unused type parameter on the catch-all overload exists only so
 * `redact<Payload>(x)` still compiles — without it TS falls through to the
 * primitive-constrained overload and errors (TS2344).
 */
...
function walk(
  ...
  // `Array.isArray` throws on a revoked Proxy; share one try with `seen.has`.
 ...
 * forms: protocol-relative resolves onto *another* host, so only the scheme
 * came from the base (`//cdn.example.com/lib.js` would otherwise lose its
 * host); and a reference with no leading slash (`cb?token=1`, `#top`) gains
 * an extra `/` from `BASE`'s empty path.
 *
 * The count runs over input normalised the way the parser normalises it
 * (leading junk stripped, tabs/newlines removed).
 *
 ...
 * that can't change how the result parses. Deliberately absent: delimiters
 * (`%#?&=/\:@+`), space, control characters, and non-ASCII (`new URL`
 * re-encodes it on reparse). Brackets and braces are the point: the default
 * `[redacted]` passes.
 *
...
/**
 * The placeholder a URL-safe mask travels in. `URLSearchParams`'s
 * percent-encode set doesn't contain `[`/`]` (so `searchParams.set` would
 * mangle a bracketed mask), while userinfo's `username`/`password` setters'
 * encode set does — so the mask is written as a placeholder both
 * serialisers pass through untouched (ASCII letters and `*`), then swapped
 * for the literal mask once serialisation is finished.
 *
 * The placeholder ends in a run of `*` longer than any run already present
 * (measured against both the serialisation and its escapes decoded one
 * level, since re-serialising can only add `%XX`/`+`, never a letter or
 * `*`), so the swap can't touch anything but the slots this pass wrote.
 * Measured against `parsed.href`, not the argument, since the parser
 ...
 * `?token=[redacted]`) so a dump stays greppable, and the result still parses
 * as a URL. See `URL_SAFE_MASK` for custom masks that must stay
 * percent-encoded instead. Every reference form keeps its shape: absolute
 * stays absolute, protocol-relative keeps `//host`, root-relative keeps its
 * leading `/`.
 *
 * A string with nothing to mask comes back byte-for-byte, since
 * `URL.toString()` normalises (adds a missing path, lowercases scheme/host,
 * punycodes) and returning that unconditionally would silently rewrite every
 * innocent URL in a dump — this is a redactor, not a canonicaliser. An
 * unparseable string falls back to a raw query rewrite.
 */
...
/*
 * An absolute URL inside free text, for `redactProse`. The lookbehind keeps
 * scanning linear on a long alphanumeric run with no `://` (naively
 * quadratic: 224ms at 40k letters, 5.3s at 200k) — same fix `TEXT_URL`
 * carries. Group 1 is the run's leading non-letters, kept *outside* the URL,
 * so `500https://x/?token=abc` masks the URL without eating the `500`.
 * `TEXT_URL` is not a drop-in here: its body also stops at a quote, letting
 * `?token="abc"` leak through, where this one's `\S+` keeps the quote inside
 * the URL. Timing regression guard is in `redact.test.ts`.
 */
const PROSE_URL = /(?<![a-z0-9+.-])([0-9+.-]*)([a-z][a-z0-9+.-]*:\/\/\S+)/gi;

/**
 * Masks credentials in free text that is about to leave the page: an error
 * message on its way into a snapshot, a `title` attribute, a status line, or
 * an agent's report.
 *
 ...
 *    masked as a value.
 * 2. Every `scheme://…` run inside the text, taken to the next whitespace,
 *    goes through `redactUrl()` — so `failed for https://x/?token=abc` comes
 *    back with `[redacted]` in place of the token.
 *
 * Never throws. The whole text becomes `UNREADABLE` when step 1 can't run at
 * all (non-string input, a `RedactOptions` field that throws when read, or a
 * text that *is* one unrewritable URL). A single URL is handed back as
 * written — the sweep otherwise carrying on — when only its own step-2
 * rewrite throws (an unparseable URL combined with a mask
 * `encodeURIComponent()` rejects); this is pinned by the `redactProse`
 * parity fixtures rather than repaired.
 *
 * This is **defence in depth for outbound error text, not a guarantee** — it
 * is *not* the substring scan `redactText()` performs, and a credential in
 * prose with no URL or assignment syntax (`auth failed: Bearer secret
 * rejected`, `password: hunter2`) is not masked, since `redact()` only masks
 * `Bearer secret` as the *whole* value, not mid-sentence. Other pinned
 * limits: a JWT glued to punctuation isn't masked; adjacent URLs with no
 * whitespace between them parse as one URL, so a later one's credential can
 * survive inside the first one's value; the body runs to the next
 * whitespace, so trailing punctuation glued to a credential is masked with
 * it.
 */
