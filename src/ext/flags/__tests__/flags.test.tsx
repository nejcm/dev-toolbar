...
describe("read-only mode", () => {
  it("offers no editors and says why", () => {
    ...
    expect(panel?.querySelector('[data-dtb-role="reset-link"]')).not.toBeNull();
  });
});
...
describe("the reset link", () => {
  ...
  it("shows no reset notice on an ordinary load", () => {
    const { toolbar } = mount({ onOverride: record });
    act(() => toolbar.openPanel("flags"));
    expect(toolbar.panel("flags")?.querySelector('[data-dtb-role="reset-notice"]')).toBeNull();
  });

  it("shows storage reset failures with an error tone", () => {
    ...
    withHistoryUrl("http://localhost/app?dtb-flags=reset", () => {
      const { toolbar } = mount({ onOverride: record }, storage);
      act(() => toolbar.openPanel("flags"));
      const notice = toolbar.panel("flags")?.querySelector('[data-dtb-role="reset-notice"]');
      expect(notice?.getAttribute("data-dtb-tone")).toBe("error");
      expect(notice?.getAttribute("role")).toBe("alert");
    });
  });

  it("shows how many overrides a reset cleared, and the reset URL for this page", async () => {
    ...
    await withHistoryUrl(
      "http://localhost:3000/?token=secret&dtb-flags=reset#section",
      async (stub) => {
        const { toolbar } = mount({ onOverride: record }, storage);
        act(() => {
          toolbar.openPanel("flags");
        ...
        expect(stub.location.search).toBe("?token=secret");
