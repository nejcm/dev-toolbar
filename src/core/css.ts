...
@layer dev-toolbar {
  ...
  /* Every element-level default below — this box-sizing, the margin resets,
     the focus ring, the button face, the field geometry — stops at a subtree
     marked data-dtb-embed. That attribute means "somebody else's UI": a
     third-party devtool panel hosted through the recipe in docs/embedding.md
     (the kit's embed() frame carries it). Such a tool brings its own CSS and
     must arrive with the UA's defaults, exactly as it would anywhere else on
     the page, not with ours. The guard is a chained :where(:not(…)) so it
     adds no specificity: each rule keeps the weight it had, and a part rule
     still outranks it. Inheritance is untouched on purpose — the frame's
     font, size and colour reach the tool as any parent's would. */
  [data-dev-toolbar],
